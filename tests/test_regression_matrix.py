...
注:
  - 抽出の「サイズ上限」（SafetyLimits → ExtractionOptions）は
    アーカイブサービス層（archive/service.py）が適用し、
    tests/test_service_limits.py で担保される。
  - RAR/7z の「subprocess 0 回」は DLL 経路（test_dll_poc.py の
    test_dll_operations_spawn_no_subprocess）で担保される。
"""
...
MATRIX: list[tuple[str, str, str]] = [
    ...
    (
        "zip.password.wrong",
        "暗号化ZIP・誤パスワードは InvalidPasswordError に分類",
        "test_unzip.py::TestZipCryptoPasswordClassification::test_extract_with_wrong_password_raises",
    ),
    (
        "zip.password.missing",
        "暗号化ZIP・未指定は PasswordRequiredError に分類",
        "test_unzip.py::TestZipCryptoPasswordClassification::test_extract_without_password_raises",
    ),
    (
        "7z.normal",
        "7z 通常の一覧・展開（実fixture）",
        "test_integration.py::Test7zIntegration::test_list",
    ),
    (
        "7z.encrypted",
        "7z パスワード付きの暗号化検出（実fixture）",
        "test_integration.py::Test7zIntegration::test_encrypted_list",
    ),
    (
        "7z.header_encrypted",
        "7z ヘッダー暗号化はオープン時にパスワード要求（DLL PoC）",
        "test_dll_poc.py::<module>::test_dll_opens_header_encrypted_7z_requires_password_at_open",
    ),
    (
        "rar.normal",
        "RAR 通常の展開（実fixture）",
        "test_integration.py::TestRarIntegration::test_extract_real_rar",
    ),
    (
        "rar.encrypted",
        "RAR パスワード付きの暗号化検出（実fixture）",
        "test_integration.py::TestRarIntegration::test_encrypted_rar_list",
    ),
    (
        "corrupt",
        "破損アーカイブのエラー",
    ...
    (
        "safety.symlink",
        "symlinkエントリは展開を拒否（脱出しない）",
        "test_unzip.py::TestSymlinkAndPathSafety::test_symlink_entry_rejected",
    ),
    (
        "safety.absolute",
        "絶対パスエントリの拒否",
...
def _test_methods(file_name: str, class_name: str) -> set[str]:
    ...
    class_name が "<module>" の場合はモジュール直下の test_* 関数を収集する。
    """
