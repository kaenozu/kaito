...
- `KAITO_UPDATE_ENDPOINT`: 認証なしで取得できる公開のRelease APIまたは同じ`tag_name`／`html_url`形式を返すJSONエンドポイント
- `KAITO_GITHUB_TOKEN`: 非公開GitHub Releasesを読むための最小権限トークン

どちらも利用できない場合は、更新確認を失敗として扱いますが、アーカイブ操作は継続します。
...
Windowsコード署名は`tools/sign_windows.ps1`とRelease workflowで行います。安定版Release workflowでは署名を必須とし、次の両方のリポジトリSecretが欠けている、証明書が無効、Code Signing EKUがない、署名検証に失敗した、のいずれかで公開前に停止します。

- `WINDOWS_CERTIFICATE_BASE64`: PFXファイルのBase64
- `WINDOWS_CERTIFICATE_PASSWORD`: PFXパスワード

ローカル開発や通常CIでは署名を任意または無効にできますが、安定版のEXEとインストーラーを未署名のまま公開することはできません。
