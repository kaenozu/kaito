...
一覧・展開・プレビュー・整合性検査の読み取り系は、同梱`7z.dll`をプロセス内で直接呼び出し、パスワードをコマンドラインに一切載せません。暗号化アーカイブの作成のみ、7-Zip CLIの仕様上パスワードが処理中のプロセス引数へ渡されます。kaitoはログ、例外、診断出力からパスワードを伏せ、永続保存もしませんが、作成時に同一Windowsユーザー権限の別プロセスから引数を参照される可能性までは排除できません。
...
- 暗号化アーカイブ作成時の7-Zip CLIプロセス引数露出リスクがあります
