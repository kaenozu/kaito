...
# 受け入れテスト専用パスワード（実データの暗号化には使わないこと。TEST_DATA.md にも明記）。
# ZIP/7z のパスワードは tests/test_encrypted_zip.py の _PASSWORD と一致させること。
# RAR のパスワードは upstream libarchive fixture に固定（tests/fixtures/rar/README.md 参照）。
# テスト専用パスワードは secret scanner の誤検知を避けるため断片を配列 join で構成する（PR #10 と同じ方針）。
$EncryptedArchivePassword = -join @('Kaito-Acceptance-', '2026!')
$EncryptedRarPassword = '12345678'
...
Invoke-SevenZip -SevenZip $sevenZip -WorkingDirectory $sourceRoot -Arguments @(
    'a', '-tzip', '-mem=AES256', "-p$EncryptedArchivePassword", (Join-Path $archivesRoot 'encrypted.zip'), '.\*'
)
Invoke-SevenZip -SevenZip $sevenZip -WorkingDirectory $sourceRoot -Arguments @(
    'a', '-t7z', '-mhe=on', "-p$EncryptedArchivePassword", (Join-Path $archivesRoot 'encrypted.7z'), '.\*'
)
...
- encrypted.zip: $EncryptedArchivePassword
- encrypted.7z: $EncryptedArchivePassword
- encrypted.rar: $EncryptedRarPassword
...
$summary = [ordered]@{
    ...
    encrypted_zip_7z_password = $EncryptedArchivePassword
    encrypted_rar_password = $EncryptedRarPassword
    archives = @(
